...
import jax
import pandas as pd
from jax_benchmarks.core.base import BenchmarkResult
from jax_benchmarks.core.config import load_config


from jax_benchmarks.core.registry import registry
...
def save_output(results: List[BenchmarkResult], output_dir: str):
...
def main(argv):
  ...
  # Import benchmarks after app.run()
  from jax_benchmarks import benchmarks

  config_path = FLAGS.config
  print(f"Loading config from: {config_path}")
  benchmark_configs = load_config(config_path)
  ...
  for cfg in benchmark_configs:
    ...
    try:
      ...
      if sys_name and "hardware_stats" not in cfg:
        from jax_benchmarks.core.system import get_system

        try:
          sys_config = get_system(sys_name)
          cfg["hardware_stats"] = {
      ...
      benchmark_cls = registry.get_benchmark(name)
