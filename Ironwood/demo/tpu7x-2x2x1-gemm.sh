...
xpk workload create --cluster "${CLUSTER}" --zone "${ZONE}" --device-type "tpu7x-8" \
  ...
  git checkout tpu7x-demo-0211 && \
  pip install -r requirements.txt && \
  export GCS_BUCKET_DIR=${GCS_PATH} && \
  ...
  --num-slices "1" --docker-image=docker.io/library/python:3.12-bookworm --workload "tpu7x-2x2x1-gemm" --project "${PROJECT}"