...
class RooflineTest(absltest.TestCase):
  ...
  def test_apply_roofline_analysis_none_mode(self):
    ...
    self.assertNotIn("roofline_tflops_limit_per_device", result)
    self.assertNotIn("wall_clock_compute_roofline_efficiency_pct", result)
    self.assertNotIn("peak_hbm_bw_per_device_gb_s", result)
    self.assertNotIn("wall_clock_memory_roofline_efficiency_pct", result)
  ...
  def test_apply_roofline_analysis_compute_mode(self):
    ...
    result = roofline.apply_roofline_analysis(self.mock_benchmark, metrics)
    self.assertAlmostEqual(result["roofline_tflops_limit_per_device"], 0.2)
    self.assertAlmostEqual(
        result["wall_clock_compute_roofline_efficiency_pct"], 25000.0
    )
    self.assertEqual(result["peak_hbm_bw_per_device_gb_s"], 200.0)
    self.assertNotIn("wall_clock_memory_roofline_efficiency_pct", result)
  ...
  def test_apply_roofline_analysis_memory_hbm_mode(self):
    ...
    self.assertEqual(result["peak_hbm_bw_per_device_gb_s"], 200.0)
    self.assertAlmostEqual(
        result["wall_clock_memory_roofline_efficiency_pct"], 40.0
    )
    self.assertNotIn("roofline_tflops_limit_per_device", result)
    self.assertNotIn("wall_clock_compute_roofline_efficiency_pct", result)

  def test_apply_roofline_analysis_zero_tflops_yields_zero_efficiency(self):
    """Test that actual_tflops=0.0 yields wall_clock_compute_roofline_efficiency_pct=0.0."""
    self.mock_benchmark.roofline_mode = constants.RooflineMode.COMPUTE
    metrics = {"wall_clock_tflops_per_device": 0.0}
    mock_hw = mock.MagicMock(spec=system.HardwareSpec)
    mock_hw.name = system.TpuVersion.TPU7X
    mock_hw.tflops = system.TflopsSpec(
        peak_tflops_per_device={"bfloat16": 100.0}
    ...
    result = roofline.apply_roofline_analysis(self.mock_benchmark, metrics)
    self.assertIn("wall_clock_compute_roofline_efficiency_pct", result)
    self.assertEqual(result["wall_clock_compute_roofline_efficiency_pct"], 0.0)
  ...
  def test_apply_roofline_analysis_flat_peak_bandwidth_invariant_to_size(self):
    ...
    self.mock_benchmark.hardware_spec = system.HardwareSpec(
        ...
        devices_per_chip=1,
        hbm=system.HbmSpec(peak_bw_gb_s=500.0),
    )
    for size in (1, 1024, 1024 * 1024, 1024 * 1024 * 1024):
      self.mock_benchmark.get_total_bytes.return_value = float(size)
      metrics = {"wall_clock_bandwidth_per_device_gb_s": 250.0}
      result = roofline.apply_roofline_analysis(self.mock_benchmark, metrics)
      self.assertEqual(result["peak_hbm_bw_per_device_gb_s"], 500.0)
      self.assertAlmostEqual(
          result["wall_clock_memory_roofline_efficiency_pct"], 50.0
      )
  ...
  def test_apply_roofline_analysis_chip_tflops_no_roofline_efficiency(self):
    ...
    self.mock_benchmark.hardware_spec = system.HardwareSpec(
        ...
        hbm=system.HbmSpec(peak_bw_gb_s=200.0),
    )
    result = roofline.apply_roofline_analysis(self.mock_benchmark, metrics)
    self.assertNotIn("wall_clock_compute_roofline_efficiency_pct", result)
  ...
  def test_apply_roofline_analysis_memory_roofline_efficiency_pct(self):
    ...
    # 2. Chip-only metric from collectives does NOT compute HBM
  ...
  def test_apply_roofline_analysis_zero_peak_bw(self):
    ...
    self.mock_benchmark.hardware_spec = system.HardwareSpec(
        name=system.TpuVersion.TPU7X,
        devices_per_chip=1,
        hbm=system.HbmSpec(peak_bw_gb_s=0.0),
    )
    metrics = {"wall_clock_bandwidth_per_device_gb_s": 50.0}
    result = roofline.apply_roofline_analysis(self.mock_benchmark, metrics)
    self.assertEqual(result["peak_hbm_bw_per_device_gb_s"], 0.0)
    self.assertNotIn("wall_clock_memory_roofline_efficiency_pct", result)
  ...
  def test_apply_roofline_analysis_hbm_none_leaves_metrics_unchanged(self):
    ...
    res_mem = roofline.apply_roofline_analysis(
        self.mock_benchmark, {"wall_clock_bandwidth_per_device_gb_s": 50.0}
    )
    self.assertNotIn("peak_hbm_bw_per_device_gb_s", res_mem)
    self.assertNotIn("wall_clock_memory_roofline_efficiency_pct", res_mem)

    # In COMPUTE mode
    self.mock_benchmark.roofline_mode = constants.RooflineMode.COMPUTE
    res_comp = roofline.apply_roofline_analysis(
        self.mock_benchmark, {"wall_clock_tflops_per_device": 50.0}
    )
    self.assertNotIn("roofline_tflops_limit_per_device", res_comp)
    self.assertNotIn("wall_clock_compute_roofline_efficiency_pct", res_comp)
  ...
  def test_apply_roofline_analysis_tpu7x_flat_peak_compute_mode(self):
    ...
    # Memory-bound shape (intensity=1.0, bfloat16)
    self.mock_benchmark.get_arithmetic_intensity.return_value = 1.0
    metrics_mem = {"wall_clock_tflops_per_device": 3.0}
    res_mem = roofline.apply_roofline_analysis(self.mock_benchmark, metrics_mem)
    self.assertEqual(res_mem["peak_hbm_bw_per_device_gb_s"], 3690.0)
    self.assertAlmostEqual(res_mem["roofline_tflops_limit_per_device"], 3.69)
    self.assertAlmostEqual(
        res_mem["wall_clock_compute_roofline_efficiency_pct"],
        (3.0 / 3.69) * 100.0,
    )

    # Compute-bound shape (intensity=400.0, bfloat16)
    self.mock_benchmark.get_arithmetic_intensity.return_value = 400.0
    metrics_compute = {"wall_clock_tflops_per_device": 1000.0}
    res_compute = roofline.apply_roofline_analysis(
        self.mock_benchmark, metrics_compute
    )
    self.assertAlmostEqual(
        res_compute["roofline_tflops_limit_per_device"], 1153.5
    )
    self.assertAlmostEqual(
        res_compute["wall_clock_compute_roofline_efficiency_pct"],
        (1000.0 / 1153.5) * 100.0,
    )
  ...
  def test_apply_roofline_analysis_tpu7x_flat_peak_memory_mode(self):
    ...
    # 2 MiB transfer (wall_clock_bandwidth_per_device_gb_s=903.75)
    self.mock_benchmark.get_total_bytes.return_value = 2 * 1024 * 1024
    metrics_2mib = {"wall_clock_bandwidth_per_device_gb_s": 903.75}
    res_2mib = roofline.apply_roofline_analysis(
        self.mock_benchmark, metrics_2mib
    )
    self.assertEqual(res_2mib["peak_hbm_bw_per_device_gb_s"], 3690.0)
    self.assertAlmostEqual(
        res_2mib["wall_clock_memory_roofline_efficiency_pct"],
        (903.75 / 3690.0) * 100.0,
    )
    self.assertAlmostEqual(
        res_2mib["wall_clock_memory_roofline_efficiency_pct"], 24.49, places=2
    )

    # 2048 MiB transfer (wall_clock_bandwidth_per_device_gb_s=3604.0)
    self.mock_benchmark.get_total_bytes.return_value = 2048 * 1024 * 1024
    metrics_2048mib = {"wall_clock_bandwidth_per_device_gb_s": 3604.0}
    res_2048mib = roofline.apply_roofline_analysis(
        self.mock_benchmark, metrics_2048mib
    )
    self.assertEqual(res_2048mib["peak_hbm_bw_per_device_gb_s"], 3690.0)
    self.assertAlmostEqual(
        res_2048mib["wall_clock_memory_roofline_efficiency_pct"],
        (3604.0 / 3690.0) * 100.0,
    )
    self.assertAlmostEqual(
        res_2048mib["wall_clock_memory_roofline_efficiency_pct"],
    ...
    self.assertLessEqual(
        res_2048mib["wall_clock_memory_roofline_efficiency_pct"], 100.0
    )
  ...
  def test_apply_roofline_analysis_fallback_dtype(self):
    ...
    self.assertIn("roofline_tflops_limit_per_device", res)
    self.assertTrue(
